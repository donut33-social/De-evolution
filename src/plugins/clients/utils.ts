...
export const isValidTweet = (tweetContent: string): boolean => {
    // Filter out tweets with too many hashtags, @s, or $ signs, probably spam or garbage
    const hashtagCount = (tweetContent.match(/#/g) || []).length;
    const atCount = (tweetContent.match(/@/g) || []).length;
    const dollarSignCount = (tweetContent.match(/\$/g) || []).length;
